...
let X = "12321";
let Y = "42531";
let result = ""; // 초기값을 ""로 주지 않으면 undefined 뜸
X = X.split("");
Y = Y.split("");
 ...
 */
  result += String(i).repeat(Math.min(curX, curY)); // i 가 x,y에 존재한다면 최소 숫자만큼 누적 더해준다.
}
if (result == "") return "-1"; // 없으면 -1
if (Number(result) == 0) return "0"; // 공통으로 가지는 숫자가 0이다.

result = result
  .split("")
  .sort((a, b) => Number(b) - Number(a))
  .join(""); // 내림차순으로 정렬해준다. (0이 있을수도 있기 때문)
