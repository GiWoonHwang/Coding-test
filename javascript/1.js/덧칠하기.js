...
// 벽의 길이에 해당하는 인덱스를 생성한 후, 0으로 초기화 한다.
const arr = Array(n).fill(0);

section.forEach((v) => (arr[v - 1] = 1)); // 칠해줘야 하는 구역을 1로 변경한다.

console.log(arr); // [ 0, 1, 1, 0, 0, 1, 0, 0 ]

let i = 0;
while (i < arr.length) {
  // 칠한곳을 또 칠해도 되니까 while문 ???
  if (arr[i] === 1) {
    // 페인트를 칠해야 할 구역이면
    result++; // 칠한 횟수 증가

    i += m; // 한번 칠할 때마다 롤러의 길이만큼 증가
  } else i++; // 칠할 필요 없으면 다음 구역~
}
