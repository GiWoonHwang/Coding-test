...
# 1단계 모든 대문자를 소문자로 치환
# 2단계 소문자,숫자,빼기,마침표,밑줄 제외 다 제거
# 3단계 .가 2번이상연속되면 .하나로 바꾼다
# 4단계 .가 처음이나 끝에 있다면 제거
# 5단계 빈 문자열이라면 a 대입
# 6단계 길이가 16자 이상이라면 15로 바꿔줌
# 7단계 길이가 2자 이하라면 3이 될때 가지 반복해서 끝에 붙임
...
# 4 
# # 4에서 if문을 또 쓰는 이유 : #4에서 문자열이 '.'인 경우 즉, len(answer) = 1인 경우 슬라이싱에서 오류가 발생하기 때문. 뒷 코드에서 마지막에 오는 '.'을 제거할 때 제거하겠다
if answer[0] == '.':
